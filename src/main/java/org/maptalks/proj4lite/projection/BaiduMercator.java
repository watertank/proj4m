...
public class BaiduMercator<T> implements Projection<T> {
    ...
    public T forward(T point, PointAdaptor<T> pointAdaptor) {
        return this.convertLL2MC(point, pointAdaptor);
    }

    public T inverse(T point, PointAdaptor<T> pointAdaptor) {
        return this.convertMC2LL(point, pointAdaptor);
    }
    ...
    private T convertMC2LL(T point, PointAdaptor<T> pointAdaptor) {
        double y_abs = Math.abs(pointAdaptor.getY(point));
        ...
        return this.convertor(point, table, pointAdaptor);
    }

    private T convertLL2MC(T point, PointAdaptor<T> pointAdaptor) {
    ...
    private T convertor(T point, double[] table, PointAdaptor<T> pointAdaptor) {
        double px = pointAdaptor.getX(point);
        double py = pointAdaptor.getY(point);
